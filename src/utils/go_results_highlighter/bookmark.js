...
function applyHighlighter(selector, settings, force) {
  const elementsWithGoResults = asArray(
  ...
  if (!elementsWithGoResults.length) {
    ...
    if (force) {
      newSelector = prompt(
        `Could not find any elements matching "${selector}" selector. Do you want to provide another one?`
      );
    } else {
      newSelector = prompt(
        `Could not find any tables with Go results ("${selector}"). If you are confident that this page has one - please provide a specific selector to the element.`
      );
    }
  ...
  elementsWithGoResults.forEach((element) => {
  ...
  console.log(
    `Go Results Highlighter was applied to ${appliedCount} DOM elements. ${skippedCount} had Highlighter before.`
  );
}
